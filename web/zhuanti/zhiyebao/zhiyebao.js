...
async function renderList(){
    let res = await fetch(`http://localhost:3000/news?category=10`).then(res=>res.json())
    console.log(res)
...
setTimeout(function(){
    search.oninput = async function(){
        ...
        <li class="list-group-item"><a href="/project/sjz/web/details/details.html?id=${item.id}">${item.title}</a></li>
        `).join("")
    }
