...
async function renderList(){
    let res = await fetch(`http://localhost:3000/news?category=6`).then(res=>res.json())
    console.log(res)
    document.querySelector("section #list").innerHTML = res.map(item=>`
    <li class="heihei" >
    <img src="${item.cover}"/ style="float:left;">
    <div>${item.title}</div>
    <div class="chakan" data-id="${item.id}">查看详情<div>
    </li>`
    ).join("")
}
document.querySelector("#list").onclick = function(evt){
    if(evt.target.className.includes("chakan")){
